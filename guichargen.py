...
class Application(tk.Frame):
    def __init__(self, charClass, master=None):
        super().__init__(master)
        self.master = master
        self.pack()
        self.create_widgets()
        self.charGen = charClass
        self.char = self.charGen(2, 2)

    def create_widgets(self):
        self.rollChar = tk.Button(self)
        self.rollChar["text"] = "Roll a Character"
        self.rollChar["command"] = self.rollAll
        self.rollChar.pack(side="top")

        self.quit = tk.Button(self, test="Quit", fg="red",
                              command=self.master.destroy)
        self.quit.pack(side="bottom")
        
    def rollAll(self):
        self.char = self.charGen(2, 2)
        self.char.NewAbil(BasicAbilTable, 4)
        self.char.rollRace(RaceTable, tables["RacialStatBonuses"], tables["RacialAbilities"])

root = tk.Tk()
app = Application(CharStats, master=root)
app.mainloop()
